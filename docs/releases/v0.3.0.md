...
This preview is ad-hoc signed and **not notarized**, so macOS reports “Murmur” Not Opened until the download's quarantine flag is removed. After moving the app into Applications, run this command in Terminal, then open Murmur:

```bash
xattr -dr com.apple.quarantine /Applications/Murmur.app
```

To avoid Terminal, try opening Murmur once, then click **Open Anyway** in System Settings → Privacy & Security. A managed Mac may block both methods.

macOS may require Accessibility permission to be granted again after replacing the app. If an enabled entry is not recognized, quit Murmur, remove the old entry in System Settings → Privacy & Security → Accessibility, add and enable the updated app, then reopen it. See the README for full installation steps.
